...
# --- CẤU HÌNH KEY SYSTEM ---
DEFAULT_API_TOKEN = "Điền Token Link4m mày vào"
SECRET_KEY_FREE = "DITCONMEMAYYYY"

def get_system_storage_path() -> str:
    """Đường dẫn lưu file key ẩn"""
    home = os.path.expanduser("~")
    if platform.system() == "Windows":
        return os.path.join(home, "AppData", "Local", "nt_system_config.dat")
    else:
        return os.path.join(home, ".nt_system_config")

STORAGE_FILE = get_system_storage_path()

def load_system_data() -> Dict[str, Any]:
    """Đọc toàn bộ file cấu hình (ID máy + Key)"""
...
def save_system_data(data: Dict[str, Any]) -> None:
    """Lưu toàn bộ cấu hình xuống file ẩn"""
    try:
        json_str = json.dumps(data)
        # Mã hóa Base64
        encoded_bytes = base64.b64encode(json_str.encode("utf-8"))
        
        # Nếu file đang ẩn trên Windows, gỡ ẩn để ghi
        ...
        console.print(f"[red]Lỗi hệ thống lưu trữ: {e}[/]")

def _generate_hardware_id_raw() -> str:
    """Hàm tạo ID gốc từ phần cứng"""
    try:
        if platform.system() == "Windows":
        ...
        # Fallback random
        random_id = hashlib.md5(str(uuid.uuid4()).encode()).hexdigest().upper()
        return f"{random_id[:4]}-{random_id[4:8]}-{random_id[8:12]}"

def get_stable_machine_id() -> str:
    """
    Lấy ID máy. 
    - Nếu đã lưu trong file ẩn -> Lấy ra (đảm bảo không đổi).
    - Nếu chưa có -> Tạo mới và lưu lại ngay lập tức.
    """
    ...
    save_system_data(data) # Lưu ngay để cố định
    return new_id

def save_license_record(key_type: str, key_value: str) -> None:
    """Chỉ cập nhật phần license, giữ nguyên machine_id"""
    data = load_system_data()
...
def get_stored_license() -> Optional[Dict[str, str]]:
    """Lấy thông tin license từ data tổng"""
    data = load_system_data()
    return data.get("license")

def clear_stored_license() -> None:
    """Chỉ xóa thông tin license, KHÔNG XÓA FILE, KHÔNG XÓA MACHINE ID"""
    data = load_system_data()
    data["license"] = None 
    save_system_data(data)

def shorten_key_link(long_url: str) -> Optional[str]:
    """Rút gọn link bằng API link4m"""
    if not long_url: 
        return None
    try:
        api = f"https://link4m.co/api-shorten/v2?api={DEFAULT_API_TOKEN}&url={long_url}"
...
def make_key_link(key_value: str) -> str:
    """Tạo link key"""
    return f"https://huy525693.github.io/tntoolkeyverify/?key={key_value}"

def get_current_date() -> str:
    """Lấy ngày hiện tại định dạng YYYYMMDD"""
    return datetime.now().strftime("%Y%m%d")

def create_free_key(session_id: str) -> str:
    """Tạo key free"""
    raw = f"{session_id}|{get_current_date()}|{SECRET_KEY_FREE}"
    hashed = hashlib.sha256(raw.encode()).hexdigest().upper()
    return f"TNtool{hashed[:19]}"

def key_activation_flow() -> bool:
    """Luồng kích hoạt key chính"""
    console = Console()
    machine_id = get_stable_machine_id()
    console.clear()
    
    console.print(Panel.fit(
        f"[bold cyan]🔐 HỆ THỐNG KEY - TNTool Escape VIP AI[/]\n"
        f"[yellow]Mã máy của bạn: {machine_id}[/]",
        border_style="cyan"
    ))
    ...
    if stored_license and stored_license.get('type') == 'FREE':
        saved_date = stored_license.get('saved_date', '')
        current_date = get_current_date()
        
        if saved_date == current_date:
            # Key còn hạn, hỏi người dùng có muốn dùng lại
            console.print(f"\n[green]✅ Phát hiện key còn hạn (ngày lưu: {saved_date})[/]")
            
            choice = input("\nBạn có muốn dùng key này tiếp không? (y/n): ").strip().lower()
            if choice == 'y':
                console.print("[green]Tiếp tục sử dụng key cũ...[/]")
                return True
            else:
                console.print("[yellow]Xóa key cũ và yêu cầu kích hoạt mới...[/]")
                clear_stored_license()
        else:
            # Key hết hạn
            console.print(f"\n[yellow]⚠️ Key đã hết hạn (lưu ngày: {saved_date}, hôm nay: {current_date})[/]")
            clear_stored_license()
    
    # Yêu cầu kích hoạt mới
    console.print("\n[yellow]🔓 Yêu cầu kích hoạt key mới (FREE 24h)[/]")
    
    while True:
        session_id = ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))
        generated_key = create_free_key(session_id)
        
        # Tạo link rút gọn
        final_link = shorten_key_link(make_key_link(generated_key))
        if not final_link:
            final_link = make_key_link(generated_key)  # Fallback
            
        console.print(f"\n[cyan]1. Truy cập link: [bold]{final_link}[/bold][/]")
        console.print("[cyan]2. Lấy key từ trang web và nhập vào đây[/]")
        
        user_input = input("\nNhập key FREE: ").strip()
        
        if user_input == generated_key:
            save_license_record('FREE', user_input)
            console.print("[green]✅ Key FREE đã được kích hoạt thành công![/]")
            time.sleep(2)
            return True
        else:
            console.print("[red]❌ Key không đúng! Vui lòng thử lại.[/]")
            retry = input("\nThử lại? (y/n): ").strip().lower()
            if retry != 'y':
                console.print("[red]Thoát tool...[/]")
                return False
...
def main() -> None:
    ...
    if not key_activation_flow():
        console.print("[red]❌ Key không hợp lệ. Thoát tool.[/]")
        sys.exit(1)
